...
{
    private static readonly string possibleLetters = "qwertyuiopasdfghjklzxcvbnm ";
    ...
    {
        var generatedOutput = new StringBuilder();
        for (var i = 0; i < targetLength; i++) generatedOutput.Append(selector.GetRandomCharacter());
    ...
    public static string GenerateMarkovWeighed(int targetLength, MarkovWeighedRandomSelector selector, int markovLength)
    {
        ...
        for (var i = prefixWord.Length; i < targetLength + prefixWord.Length; i++)
            generatedOutput.Append(
                selector.GetRandomCharacter(generatedOutput.ToString().Substring(i - markovLength, markovLength)));
